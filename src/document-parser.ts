...
export class DocumentParser {
	...
	parseDocumentFile(xmlDoc: Element): DocumentElement {
		...
		return {
			...
			props: sectPr
				? parseSectionProperties(sectPr, xml)
	...
	parseBodyElements(element: Element): OpenXmlElement[] {
		...
		for (let elem of xml.elements(element)) {
			switch (elem.localName) {
				...
				case "sdt":
					children.push(
						...this.parseSdt(elem, (e) => this.parseBodyElements(e))
					);
					break;
			}
		}
			...
			switch (el.localName) {
				...
				case "commentRangeStart":
					result.children.push(
						new WmlCommentRangeStart(xml.attr(el, "id"))
					);
					break;

				case "commentRangeEnd":
					result.children.push(
						new WmlCommentRangeEnd(xml.attr(el, "id"))
					);
					break;
				...
				case "sdt":
					result.children.push(
						...this.parseSdt(
							el,
							(e) => this.parseParagraph(e).children
						)
					);
					break;
				...
				case "ins":
					result.children.push(
						this.parseInserted(el, (e) => this.parseParagraph(e))
					);
					break;

				case "del":
					result.children.push(
						this.parseDeleted(el, (e) => this.parseParagraph(e))
					);
					break;
			}
		}
		...
		xmlUtil.foreach(node, (c) => {
			...
			switch (c.localName) {
				...
				case "commentReference":
					result.children.push(
						new WmlCommentReference(xml.attr(c, "id"))
					);
					break;
	...
	parseTable(node: Element): WmlTable {
		...
		xmlUtil.foreach(node, (c) => {
			switch (c.localName) {
				case "tr":
					const row = this.parseTableRow(c);
					if (row["gridBefore"]) {
						row.children[0]["span"] =
							(row.children[0]["span"] ?? 1) + row["gridBefore"];
					}
					if (row["gridAfter"]) {
						row.children[row.children.length - 1]["span"] =
							(row.children[row.children.length - 1]["span"] ??
								1) + row["gridAfter"];
					}
		...
		this.parseDefaultProperties(
			...
			(c) => {
				switch (c.localName) {
					...
					case "tblLayout":
						table.cssStyle["table-layout"] = xml.attr(c, "type");
						break;
	...
	parseTableRowProperties(elem: Element, row: WmlTableRow) {
		row.cssStyle = this.parseDefaultProperties(elem, {}, null, (c) => {
			switch (c.localName) {
				...
				case "gridBefore": // 复杂表格有些行会通过这个设置隐藏列,导致表格渲染异常
					row["gridBefore"] = parseInt(xml.attr(c, "val"));
					break;

				case "gridAfter":
					row["gridAfter"] = parseInt(xml.attr(c, "val"));
					break;
	...
	parseDefaultProperties(
		...
		xmlUtil.foreach(elem, (c) => {
			...
			switch (c.localName) {
				...
				case "smallCaps":
					style["font-variant"] = xml.boolAttr(c, "val", true)
						? "small-caps"
						: "none";
					break;
	...
	parseFont(node: Element, style: Record<string, string>) {
		...
		var fonts = [ascii, asciiTheme, eastAsia]
			.filter((x) => x)
